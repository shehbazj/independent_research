...
# Setup Ubuntu VM using VirtualBox
...
##### At this point, please email the TA with the output of all checkpoints in a doc/pdf file. Proceed only once the TA has acknowledged that everything is working as expected. Email your checkpoint results in an email with subject “CSC494 - results 1”. Only 1 email is required per group.
...
```
This outputs a lot of information. Read, understand and identify each column. What does each column stand for? Can you combine all blktrace files and produce 1 consolidated output? 
...
Can you add one (or more) masks (-a) to blktrace to reduce the size of the blocktrace? Re-run with appropriate masks, analyze the change in number of blocks that get logged, and report your results in class.
